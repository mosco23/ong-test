@php
    $orgCharts = Illuminate\Support\Facades\DB::table('org_charts')
                                            ->join('users','users.id','=','org_charts.user_id')
                                            ->select("org_charts.id", "title", "pid", "img", "users.name as name")
                                            ->get()
                                            ->sortBy('rank');

    $orgCharts = $orgCharts->map(function($org){
...
<section class="bg-white py-14">
    ...
    <script>
        var nodes =  {!! json_encode($orgCharts) !!};

        for (var i = 0; i < nodes.length; i++) {
            var node = nodes[i];
            switch (node.title) {
                case "SECRETAIRE":
                    node.tags = ["SECRETAIRE"];
                    break;
                case "TRESORIER":
                    node.tags = ["TRESORIER"];
                    break;
                case "COORDONNATEUR":
                    node.tags = ["COORDONNATEUR"];
                    break;
                case "COMMISSAIRE AUX COMPTES":
                    node.tags = ["COMMISSAIRE AUX COMPTES"];
                    break;
            }
        }
        ...
        var chart = new OrgChart(document.getElementById("tree"), {   
            ...
            nodeMouseClick: OrgChart.action.collapse,
            enableSearch: false,
            showXScroll: true,
            showYScroll: true,
            filterBy: ['rank'],
